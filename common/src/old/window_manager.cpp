...
namespace window
{
    ...
    {
        unique_GLFWwindow glfwWindow{ glfwCreateWindow(width, height, title.c_str(), nullptr, nullptr), &glfwDestroyWindow };
        if (!glfwWindow) {
        ...
        return Window{ id, windowHandle, { .m_title = title, .m_width = width, .m_height = height, .m_cursorPos = {} } };
    }
