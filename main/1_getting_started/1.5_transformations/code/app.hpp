...
public:
    ...
    static inline auto s_assets_path = util::assets_path("1.5_transformations");
...
private:
    ...
    UniformData<float> u_mixValue{ "u_mixValue", 0.5f };
    UniformData<bool>  u_invertColor{ "u_invertColor", false };
    UniformData<bool>  u_multiplyColor{ "u_multiplyColor", false };
        ...
        }
    {
        ...
        glfwSetKeyCallback(m_window.get(), [](GLFWwindow* window, int key, int /* scancode */, int action, int /* mods */) {
            ...
            if (key == GLFW_KEY_C && action == GLFW_PRESS) {
                auto& [name, value]{ app.u_multiplyColor };
                if ((value = !value)) {
                    app.m_shader.setUniform(name, true);
                } else {
                    app.m_shader.setUniform(name, false);
...
private:
    ...
    void renderLoop()
    {
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            trans = glm::rotate(trans, std::sin((float)glfwGetTime()), { 0.0f, 0.0f, 1.0f });
            trans = glm::scale(trans, { 0.75f, 0.75f, 0.75f });
            m_shader.setUniform("u_transform", trans);
            ...
            // do some transformations again
            glm::mat4 trans2{ 1.0f };
            trans2 = glm::translate(trans2, { std::sin(glfwGetTime() * 3), std::cos(glfwGetTime() * 2), 0.0f });
            trans2 = glm::rotate(trans2, (float)std::tan(glfwGetTime()), { 0.0f, 0.0f, 1.0f });
            trans2 = glm::scale(trans2, { 0.25f, 0.25f, 0.25f });
            m_shader.setUniform("u_transform", trans2);
