...
private:
    ...
    float     m_cameraYaw{ 360.0f - 90.0f };
...
private:
    ...
    void setCallbacks()
    {
        ...
        glfwSetCursorPosCallback(m_window.get(), [](GLFWwindow* window, double xpos, double ypos) {
            ...
            if (!app.m_captureMouse) { return; }

            float sensitivity  = 0.1f;
            xoffset           *= sensitivity;
            yoffset           *= sensitivity;

            app.m_cameraYaw   = std::fmod(360.0f + app.m_cameraYaw + xoffset, 360.0f);    // how to keep it in range [0, 360]?
            app.m_cameraPitch = std::clamp(app.m_cameraPitch + yoffset, -89.0f, 89.0f);

            glm::vec3 direction;
            direction.x       = cos(glm::radians(app.m_cameraYaw)) * cos(glm::radians(app.m_cameraPitch));
            direction.y       = sin(glm::radians(app.m_cameraPitch));
            direction.z       = sin(glm::radians(app.m_cameraYaw)) * cos(glm::radians(app.m_cameraPitch));
        ...
        glfwSetScrollCallback(m_window.get(), [](GLFWwindow* window, double /* xoffset */, double yoffset) {
            ...
            app.m_cameraFov += -static_cast<float>(yoffset) * app.m_cameraSensitivity;
            app.m_cameraFov  = std::clamp(app.m_cameraFov, 1.0f, 180.0f);
        });
    ...
    void setDefaultKeyEventHandler()
    {
        ...
        {    // camera movements
            addKeyEventHandler(GLFW_KEY_W, 0, KeyEventHandler::KeyActionType::CONTINUOUS, [this] {
                m_cameraPos += (float)m_deltaTime * m_cameraSpeed * m_cameraFront;
            });
            ...
            addKeyEventHandler(GLFW_KEY_A, 0, KeyEventHandler::KeyActionType::CONTINUOUS, [this] {
                m_cameraPos -= (float)m_deltaTime * glm::normalize(glm::cross(m_cameraFront, m_cameraUp)) * m_cameraSpeed;
            });
